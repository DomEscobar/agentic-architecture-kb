...
INDEX_LANES = {
    "RAG and knowledge systems": (
        ...
        "synthesis-rag-current-evidence-2026-08",
        "pattern-bounded-rag-architecture-search",
        "pattern-retrieval-context-technique-catalog",
        "pattern-document-centric-hybrid-rag",
    ),
    "Agentic runtime and orchestration": (
        "synthesis-agentic-runtime-techniques",
        "pattern-runtime-decision-guide",
        "pattern-runtime-build-vs-adopt",
        "pattern-runtime-safety-baseline",
    ),
    "Agentic memory": (
        "synthesis-agentic-memory-architecture",
        "pattern-agent-memory-framework-selection",
        "pattern-memory-write-and-promotion",
        "pattern-memory-read-routing",
        "pattern-verifiable-memory-forgetting",
    ),
    "Evaluations and observability": (
        "synthesis-agent-evaluation-techniques",
        "synthesis-evaluation-workload-blueprints",
        "pattern-evidence-first-agent-evaluation",
        "pattern-llm-judge-calibration",
        "pattern-online-evaluation-and-rollout",
    ),
    "Security": (
        "pattern-agentic-runtime-security-boundary",
        "pattern-rag-security-and-retrieval-governance",
        "pattern-agentic-rag-security-architecture",
        "pattern-mcp-and-agent-extension-security",
        "pattern-agent-sandbox-selection",
        "pattern-agent-security-evaluation-and-tooling",
    ),
...
def render_navigation_index(
    pages: list[Page], techniques: list[dict[str, Any]], claim_count: int
) -> str:
    """Render the tracked human/agent navigation projection from canonical data."""
    by_id = {page.metadata["id"]: page for page in pages}
    missing = sorted(
        page_id
        for page_ids in INDEX_LANES.values()
        for page_id in page_ids
        if page_id not in by_id
    )
    if missing:
        raise ValueError(f"navigation lane references unknown page ids: {', '.join(missing)}")

    revision = max(page.metadata["updated_at"] for page in pages)
    lines = [
        "# Knowledge Map",
        "",
        ...
        "",
        f"Revision: `{revision}` · {len(pages)} pages · {claim_count} claims · {len(techniques)} technique cards",
        "",
        "## Knowledge lanes",
        "",
    ]
    for lane, page_ids in INDEX_LANES.items():
        lines.extend([f"### {lane}", ""])
        for page_id in page_ids:
            page = by_id[page_id]
            lines.append(f"- [{page.metadata['title']}]({page.relative_path})")
        lines.append("")
    ...
    for page_type, heading in (
        ...
        selected = sorted(
            (page for page in pages if page.metadata["type"] == page_type),
            key=lambda page: (page.metadata["title"].casefold(), page.metadata["id"]),
        )
    ...
    technique_groups: dict[str, int] = {}
    for technique in techniques:
        path = technique.get("_path", "")
        parts = Path(path).parts
        group = parts[1] if len(parts) > 2 and parts[0] == "techniques" else "other"
        technique_groups[group] = technique_groups.get(group, 0) + 1
    lines.extend(["### Technique cards", ""])
    for group, count in sorted(technique_groups.items()):
        target = "techniques/" if group == "other" else f"techniques/{group}/"
        lines.append(f"- [{group.replace('-', ' ').title()}]({target}) — {count} cards")
    lines.extend([
        "",
        "## Machine interface",
        "",
...
def lint() -> dict[str, Any]:
    ...
    for technique in techniques:
        ...
        technique_payload = {key: value for key, value in technique.items() if key != "_path"}
        for issue in sorted(technique_validator.iter_errors(technique_payload), key=lambda item: list(item.path)):
    ...
    try:
        ...
        if not NAVIGATION_INDEX_PATH.exists():
            errors.append("index.md: missing generated knowledge map")
        elif NAVIGATION_INDEX_PATH.read_text(encoding="utf-8") != expected_navigation:
            errors.append("index.md: generated knowledge map is stale; run 'python3 tools/wiki.py navigation'")
...
def compile_wiki() -> dict[str, Any]:
    ...
    techniques, technique_errors = load_techniques()
    if page_errors or technique_errors:
        return {"ok": False, "errors": sorted([*page_errors, *technique_errors])}
    claim_count = sum(
        1 for line in CLAIM_LEDGER_PATH.read_text(encoding="utf-8").splitlines() if line.strip()
    )
    # Refresh the tracked navigation projection first; lint then verifies that
    # it exactly matches the same canonical inputs used for compilation.
...
def main() -> int:
    ...
    parser.add_argument("command", choices=("lint", "compile", "index", "navigation", "search"))
    ...
    if args.command == "compile":
        ...
        pages, page_errors = load_pages()
        techniques, technique_errors = load_techniques()
        claim_count = sum(1 for line in CLAIM_LEDGER_PATH.read_text(encoding="utf-8").splitlines() if line.strip())
        errors = [*page_errors, *technique_errors]
